...
//arg0 = addresses of original registers
//arg1 = pending interrupts
uint32_t *irq(uint32_t *regs, uint32_t irqs)
{
	...
	//the LSB of q0 is set when the interrupted instruction is a compressed instruction => regs[0] & 1 = 1
	if ((irqs & 6) != 0) {
		uint32_t pc = (regs[0] & 1) ? regs[0] - 3 : regs[0] - 4;
		uint32_t instr = *(uint16_t*)pc;

		// Opcodes with the lower two bits set correspond to instructions wider than 16 bits
		if ((instr & 3) == 3)
